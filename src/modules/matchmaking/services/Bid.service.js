...
const withdrawBidService = async (handymanId, jobId, bidId) => {
    ...
    try {
        ...
        if (remainingActiveBids === 0) {
            const job = await Job.findByPk(jobId, { transaction: t });
            if (job && job.current_status === 'BIDDING') {
...
const acceptBidService = async (customerId, jobId, bidId) => {
    const t = await db.transaction();
    try {
        const job = await Job.findByPk(jobId, { transaction: t });
        if (!job) {
        ...
        if (job.customer_id !== customerId) {
            await t.rollback();
            return { EM: "You do not have permission to accept bids on this job.", EC: 403, DT: "" };
        }
        ...
        const winningBid = await Bid.findOne({
            where: { id: bidId, job_id: jobId, status: 'PENDING' },
            transaction: t
        });
        if (!winningBid) {
            await t.rollback();
            return { EM: "Bid not found or is no longer available.", EC: 404, DT: "" };
        }

        await winningBid.update({ status: 'WON' }, { transaction: t });

        await Bid.update(
            { status: 'LOST' },
            { where: { job_id: jobId, status: 'PENDING' }, transaction: t }
        );

        await job.update({
            selected_handyman_id: winningBid.handyman_id,
            current_status: 'ACCEPTED',
            final_agreed_price: winningBid.proposed_price
        }, { transaction: t });
        ...
        return {
            EM: "Bid accepted. Job is now confirmed and the handyman has been notified.",
            EC: 0,
            DT: {
                job: updatedJob,
                accepted_bid: winningBid
            }
        };
    } catch (error) {
        await t.rollback();
        console.log(">>> Error in acceptBidService: ", error);
        return { EM: "Internal server error while accepting bid.", EC: 500, DT: "" };
    }
};
...
const getMyBidsService = async (handymanId) => {
    try {
            ...
            include: [
                {
                    ...
                    attributes: [
                        ...
                        'estimated_budget_min', 'estimated_budget_max', 'final_agreed_price', 'createdAt'
                    ],
                    include: [
                        {
                            model: Service,
        ...
        return { EM: "My bids retrieved successfully.", EC: 0, DT: bids };
    } catch (error) {
...
export { submitBidService, updateBidService, withdrawBidService, acceptBidService, getMyBidsService };
