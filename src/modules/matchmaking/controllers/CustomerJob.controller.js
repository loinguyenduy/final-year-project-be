    ...
    try {
        ...
        if (estimated_budget_min !== undefined && estimated_budget_max !== undefined) {
            if (Number(estimated_budget_min) < 0) {
                return res.status(400).json({ EM: "Minimum budget cannot be negative.", EC: 1, DT: "" });
            }
            if (Number(estimated_budget_min) > Number(estimated_budget_max)) {
                return res.status(400).json({ EM: "Minimum budget cannot be greater than maximum budget.", EC: 1, DT: "" });
            }
        }
        ...
        if (parsedAddressOption === 1 && (!detail_address || !province_code || !ward_code)) {
            return res.status(400).json({ EM: "Missing detail_address, province_code or ward_code for option 1.", EC: 1, DT: "" });
        } else if (parsedAddressOption === 2) {
            // No extra fields needed, will fetch from User profile
        } else if (parsedAddressOption === 3 && (!gps_lat || !gps_long || gps_lat === "null" || gps_long === "null")) {
            return res.status(400).json({ EM: "Missing gps_lat or gps_long for option 3.", EC: 1, DT: "" });
        } else if (![1, 2, 3].includes(parsedAddressOption)) {
            return res.status(400).json({ EM: "Invalid address_option. Must be 1, 2, or 3.", EC: 1, DT: "" });
        }
